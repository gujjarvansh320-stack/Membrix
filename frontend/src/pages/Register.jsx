...
//   const [plan, setPlan] = useState('basic'); // ✅ Added plan state
...
//     try {
//       // ✅ Pass the selected plan to the backend
//       await registerUser({ name, email, password, role: 'owner', plan });
//       navigate('/login');
//     } catch (err) {
//       setError(err);
//     }
...
//         <h2 className="text-2xl font-bold text-center text-gray-800 mb-6">Register Page</h2>
...
const Register = () => {
  ...
  // ✅ Automatically lock the plan to 'basic' if they select anything other than a gym
  useEffect(() => {
    if (orgType !== 'gym') {
      setPlan('basic');
    }
  ...
  const handleRegister = async (e) => {
    ...
    try {
      await registerUser({ name, email, password, role: 'owner', plan, orgType });
      navigate('/login');
    } catch (err) {
      setError(err);
    }
